...
function fun_set_ngrok_user_env(){
    ...
    read -p "(single-user please input: Y, multi-user input N, Default [N]):" str_single_user
...
case "$action" in
    install)
        rm -f ${str_dir_shell}/ngrok-install.log
        fun_install_ngrok 2>&1 | tee ${str_dir_shell}/ngrok-install.log
    ;;
    install)
        rm -f ${str_dir_shell}/ngrok-install.log
        fun_install_ngrok 2>&1 | tee ${str_dir_shell}/ngrok-install.log
